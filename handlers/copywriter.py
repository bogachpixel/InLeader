...
async def process_copy_preset(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        # 3. ГЕНЕРАЦИЯ ТЕКСТА
        gen = await ai_service.generate_text(prompt, get_system_instruction(uid), task_type="copywriter")
        
        if gen.text.startswith("⚠️"):
            await wait_msg.edit_text(gen.text, parse_mode=None)
            return
